...
delay = 0.2;
amp = 1;
arrayCaptado = echo_gen(arrayMusica, Fs, delay, amp);
%---------------------------------------------------
...
num_coef = 300;    %número de coeficientes
...
%-----------------------ITERAÇÕES--------------------------------------------------
	...
	for(i=1:num_ite)        %loop do num de coeficientes até num de iterações 	   
	    ...
	    y = w'*xk;          %filtro 
	    e = d(i)- y;          % erro = (sinal filtrado + ruído) - saída do filtro adaptativo 
	    %y=w'*x(i+Ns:i+num_coef+Ns); %Formando a extimativa do ruído (y).
	    %e = d(i+num_coef) - y; %Subtraindo do sinal a extimativa do ruído.
	    ...
	    if(E<LMult*El) %Verifica, pela potência do sinal, se há voz. Havendo, não atualizamos os coeficientes.
	       	%w=w+u*err(k)*d(i+Ns:i+N+Ns);%Atualiza os coeficientes do filtro.
